...
@class TuTaoUITextField;

@protocol TuTaoUITextFieldDelegate <UITextFieldDelegate>
 ...
 */
- (void)textField:(TuTaoUITextField *)textField didPreventTextChangeInRange:(NSRange)range replacementString:(NSString *)replacementString;
 ...
 */
@interface TuTaoUITextField : UITextField

@property(nonatomic, weak) id<TuTaoUITextFieldDelegate> delegate;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (TuTao) tu_lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */

...
#define UIColorMakeWithHex(hex) [UIColor tu_colorWithHexString:hex]

@interface UIColor (TuUI)
 ...
 * @return UIColor对象
 */
+ (UIColor *)tu_colorWithHexString:(NSString *)hexString;

/**
 *  将当前色值转换为hex字符串，通道排序是AARRGGBB（与Android保持一致）
 */
- (NSString *)tu_hexString;
 ...
 */
- (CGFloat)tu_saturation;

/**
 *  获取当前UIColor对象里的brightness（亮度）
 */
- (CGFloat)tu_brightness;
 ...
 */
- (UIColor *)tu_colorWithoutAlpha;

/**
 *  计算当前color叠加了alpha之后放在指定颜色的背景上的色值
 */
- (UIColor *)tu_colorWithAlpha:(CGFloat)alpha backgroundColor:(UIColor *)backgroundColor;

/**
 *  计算当前color叠加了alpha之后放在白色背景上的色值
 */
- (UIColor *)tu_colorWithAlphaAddedToWhite:(CGFloat)alpha;
 ...
 */
- (UIColor *)tu_transitionToColor:(UIColor *)toColor progress:(CGFloat)progress;
 ...
 */
- (UIColor *)tu_inverseColor;

/**
 *  判断当前颜色是否等于系统默认的 tintColor 颜色。
 *  背景：如果将一个 UIView.tintColor 设置为 nil，表示这个 view 的 tintColor 希望跟随 superview.tintColor 变化而变化，所以设置完再获取 view.tintColor，得到的并非 nil，而是 superview.tintColor 的值，而如果整棵 view 层级树里的 view 都没有设置自己的 tintColor，则会返回系统默认的 tintColor（也即 [UIColor tu_systemTintColor]），所以才提供这个方法用于代替判断 tintColor == nil 的作用。
 */
- (BOOL)tu_isSystemTintColor;

/**
 *  获取当前系统的默认 tintColor 色值
 */
+ (UIColor *)tu_systemTintColor;
 ...
 */
+ (UIColor *)tu_colorWithBackendColor:(UIColor *)backendColor frontColor:(UIColor *)frontColor;
 ...
 *  @param progress 变化程度，取值范围0.0f~1.0f
 */
+ (UIColor *)tu_colorFromColor:(UIColor *)fromColor toColor:(UIColor *)toColor progress:(CGFloat)progress;

/**
 *  产生一个随机色，大部分情况下用于测试
 */
+ (UIColor *)tu_randomColor;

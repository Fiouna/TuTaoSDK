...
@class TuTaoUITextView;

@protocol TuTaoUITextViewDelegate <UITextViewDelegate>
 ...
 */
- (void)textView:(TuTaoUITextView *)textView newHeightAfterTextChanged:(CGFloat)height;
 ...
 */
- (BOOL)textViewShouldReturn:(TuTaoUITextView *)textView;
 ...
 */
- (void)textView:(TuTaoUITextView *)textView didPreventTextChangeInRange:(NSRange)range replacementText:(NSString *)replacementText;
 ...
 */
@interface TuTaoUITextView : UITextView<TuTaoUITextViewDelegate>

@property(nonatomic, weak) id<TuTaoUITextViewDelegate> delegate;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (TuTaoUI) TuTaoui_lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
